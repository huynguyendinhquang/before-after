...
class PostgresRoute:
    """One parsed route used identically by SQLAlchemy, psycopg, and libpq."""
...
def _route(value: object) -> PostgresRoute:
    ...
    userinfo = f"{username}{encoded_password}@" if user is not None or password is not None else ""
    ...
    if authority_host is None and not userinfo:
...
def sanitized_postgres_environment(value: object, *, base: dict[str, str] | None = None) -> dict[str, str]:
    """Return an environment whose PostgreSQL routing comes only from `value`."""
    route = postgres_route(value)
    environment = dict(os.environ if base is None else base)
    for key in tuple(environment):
        if key.startswith(PG_ENV_PREFIX):
            environment.pop(key, None)
    environment.update(route.environment())
    environment["DATABASE_URL"] = route.sqlalchemy_url
    # Passwords travel via a temporary PGPASSFILE for native clients, not env.
...
def scoped_postgres_environment(value: object):
    ...
    route = postgres_route(value)
    previous = {key: os.environ[key] for key in os.environ if key.startswith(PG_ENV_PREFIX) or key == "DATABASE_URL"}
    for key in tuple(previous):
        os.environ.pop(key, None)
    os.environ.update(route.environment())
    os.environ.pop("PGPASSWORD", None)
    os.environ["DATABASE_URL"] = route.sqlalchemy_url
    try:
        yield route
    finally:
